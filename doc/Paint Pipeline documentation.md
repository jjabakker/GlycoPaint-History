...
automatically, if not the user has to provide this information manually.
...
shown below. The Threshold parameter is necessary for TrackMate
...
230531 experiment is shown below..
...
the recordings in the Exepriment folder (or for all experiments in a Project 
folder) are analysed by superimposing a grid over the image and determining the
...
value of 1 indicates a perfect fit, values lower than 0.5 that a fit was
...
The Viewer dialogue appears, showing on the left the image
...
using the Select Squares dialogue. Sliders enable you to set
