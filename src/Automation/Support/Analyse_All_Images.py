...
def analyse_all_images(paint_directory):
    ...
    for index, row in df_batch.iterrows():
        ...
        for cell_id in cell_ids:
            ...
            df_cell_visible = df_cell[df_cell["Visible"] == True]
            if len(df_cell_visible) == 0:
                ...
                if len(taus) > 0:
                    ...
                    if len(taus) >= 2:
                        ...
                        tau_std   = 0
                    tau_nr        = len(taus)
                    row           = [image_name, cell_id, cell_type, probe_type, probe, concentration, threshold, key, tau_mean, tau_std, tau_nr]
                    df_stats      = pd.concat([df_stats, pd.DataFrame.from_records([row])])
                    count += 1
                    print(f" {image_name:32s}-{cell_id:1d}  {cell_type:10} {probe_type:5s} {probe:6s} {concentration:2.1f} {threshold:3.1f}: {tau_mean:8.0f} {tau_std:8.0f} {tau_nr:8d}")
        print('\n')
